...
//	printf("SubstrateHookFunctionThumb\n");
    ...
    if ((align == 0 || area[0] == T$nop) &&
        ...
        if (result != NULL) *result = reinterpret_cast<void *>(arm[1]);
...
#if 0
				if ((start & 0x1) == 0) buffer[start++] = T$nop;
...
//				buffer[start + 1] = T2$ldr_rt_$rn_im$(exts.rt, A$pc, T$Label(start + 0, end - 2));
				// fix for warning : taking the absolute value of unsigned type 'unsigned int' has no effect
				// from https://github.com/rrrfff/AndHook
				buffer[start + 1] = T2$ldr_rt_$rn_im$(exts.rt, A$pc, T$Label(static_cast<intptr_t>(start + 0), static_cast<intptr_t>(end - 2)));
...
__label_fail:
		...
		for (unsigned offset(0); offset != used / sizeof(uint32_t); ++offset) {
			if (A$pcrel$r(backup[offset])) {
				...
				if (guard) buffer[start++] = A$stmdb_sp$_$rs$((1 << copy.rn));

				// fix for warning : taking the absolute value of unsigned type 'unsigned int' has no effect
				// from https://github.com/rrrfff/AndHook
				intptr_t imm      = static_cast<intptr_t>(end - 1 - (start + 0)) * 4 - 8;
				buffer[start + 0] = A$ldr_rd_$rn_im$(copy.rn, A$pc, imm);
//				buffer[start + 0] = A$ldr_rd_$rn_im$(copy.rn, A$pc, (end - 1 - (start + 0)) * 4 - 8);
				buffer[start + 1] = copy.value;

				start += 2;

				if (guard) buffer[start++] = A$ldmia_sp$_$rs$((1 << copy.rn));
...
static void SubstrateHookFunction(SubstrateProcessRef process, void *symbol, void *replace, void **result) {
    ...
    if (result != NULL) {
		...
		for (size_t offset(0), width; offset != used; offset += width) {
			...
			} else if (backup[offset] == 0xe3 ||
					   (backup[offset] & 0xf0) == 0x70) { // XXX: opcode2 & 0xf0 is 0x80?
				length += decode.len;
				length += MSSizeOfJump(area + offset + decode.len + *reinterpret_cast<int8_t *>(backup + offset + 1));
			}
		}
