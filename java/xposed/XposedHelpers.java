 ...
 */
@SuppressWarnings({"unused", "WeakerAccess", "CaughtExceptionImmediatelyRethrown"})
public final class XposedHelpers {
    private static class XC_MethodReplacement {
    ...
    private static class XC_MethodHook {
        ...
        public static final class MethodHookParam {
            MethodHookParam(final Object thisObject, final Object[] args) {
        ...
        public static final class Unhook {
            ...
            Unhook(final XC_MethodHook callback, final int slot) {
            ...
            public void unhook() {
                // not yet implemented
                AndHook.recover(this.slot, null);
            }
        }

        protected static Object start(final Object thiz, final Object... args) throws Throwable {
            final XC_MethodHook xc = (XC_MethodHook) thiz;
            final MethodHookParam param = new MethodHookParam(thiz, args);
    ...
    private static final class XposedBridge {
        static Method getReplacement(final XC_MethodHook callback, final String methodName,
                                     final Class<?>... parameterTypes) {
            return XposedHelpers.findMethodExact(callback.getClass(), methodName, parameterTypes);
        }

        static XC_MethodHook.Unhook hookMethod(final Method m, final XC_MethodHook callback) {
            final Class<?> clazz = m.getDeclaringClass();
            final Class<?>[] parameterTypes = m.getParameterTypes();
            final String signature = HookHelper.getConstructorSignature(clazz, parameterTypes);
            final Method replacement = getReplacement(callback, "init", parameterTypes);
            final int slot = AndHook.hook(clazz, HookHelper.constructorName, signature, replacement);
            return new XC_MethodHook.Unhook(callback, slot);
        }

        static XC_MethodHook.Unhook hookMethod(final Constructor<?> c, final XC_MethodHook callback) {
            final Class<?> clazz = c.getDeclaringClass();
